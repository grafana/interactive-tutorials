...
4. **Scaffold content files.** Create `content.json` for every milestone — interactive blocks for UI steps, markdown blocks for conceptual content. Gate any screenshot, embedded video, or markdown image for website-only rendering: wrap dedicated `image`/`video` blocks in a `conditional` with `conditions: ["renderer:website"]` and `whenFalse: []`; for markdown that mixes prose with `![alt](url)`, use a dual-branch conditional (`whenTrue` = markdown with images, `whenFalse` = the same prose with the image syntax removed). Don't gate plain YouTube text links or `website.yaml` `cta.image`. See [Screenshots and videos](../../docs/learning-path-authoring.md#screenshots-and-videos-website-only-in-pathfinder).
