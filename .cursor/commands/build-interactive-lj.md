...
5. **`business-value` is markdown-only.** The `business-value` milestone always uses markdown blocks exclusively. Never add interactive, section, guided, or multistep blocks to it. A `conditional` block is allowed **only** when both `whenTrue` and `whenFalse` contain markdown blocks and nothing else (for example, gating a screenshot/video embedded in its markdown per Rule 4) — that doesn't violate the markdown-only rule because neither branch adds an interactive, section, guided, or multistep block.
