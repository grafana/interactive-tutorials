...
Every generated markdown file in `assets/` **must** start with this frontmatter block:
...
dashboardSha256: {sha256 of dashboard-source.json}
---
```

Compute `dashboardSha256` by running `shasum -a 256 {guide_dir}/assets/dashboard-source.json` and using the hex digest.
...
dashboard:
  ...
  panel_count: {N}
  source_sha256: "{sha256 of dashboard-source.json}"
...
**Write your results** to `{guide_dir}/assets/extraction-report.md`. Start the file with this frontmatter:
...
Start the file with the standard frontmatter disclaimer (see "Generated Files" section), then use this structure:
