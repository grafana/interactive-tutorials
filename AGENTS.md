...
Each snippet is a flat `shared/snippets/<id>.json` file with `id`, `title`, `description`, and `blocks`. Just commit the body — `shared/snippets/index.json` is regenerated by the deploy workflow before pushing to the CDN, so it's never committed (same pattern as `repository.json`).

CI runs `scripts/build-snippets-index.py` on every PR to catch malformed bodies (missing required fields, `id` not matching the filename). See [shared/snippets/README.md](shared/snippets/README.md) for the full schema.
